...
class AWSProfileGenerator():
    ...
    def __init__(self):
        ...
        self.ext_cfg = self.homedir.joinpath('.aws').joinpath('config-chrome-ext')
        ...
        if not os.path.exists(self.ext_cfg):
            print('\nNo ' + str(os.path.split(self.ext_cfg)[1]) + ' found. Creating...' + str(self.ext_cfg))
            open(self.ext_cfg, "w").close()
